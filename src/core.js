...
export function canonicalPoster(raw, allowedHosts = DEFAULT_IMAGE_HOSTS) {
  ...
  try {
    const u = new URL(raw);
    if (!/\/poster-cache\/proxy\/poster\//.test(u.pathname)) return raw;
    const key = u.searchParams.get("key") || "";
    if (key.toUpperCase().startsWith("TP-")) return raw;
...
export function operationFor(item, meta, allowedHosts = DEFAULT_IMAGE_HOSTS) {
  ...
  const nextPoster = canonicalPoster(meta.poster, allowedHosts);
  if (!safePoster(nextPoster, allowedHosts) || item.poster === nextPoster) return null;
  return {
    id: item._id,
    type: item.type,
    ...
    reason: nextPoster !== meta.poster ? "canonical-fallback" : "metadata-current"
  };
}
...
export async function applyOperation({authKey, account, operation, allowedHosts = DEFAULT_IMAGE_HOSTS, fetchImpl = fetch}) {
  ...
  assert(safePoster(operation.nextPoster, allowedHosts), "POSTER_HOST_NOT_ALLOWED");
